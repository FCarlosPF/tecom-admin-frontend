...
import { getAllEmployees, addEmpleado, updateEmpleado, deleteEmpleado } from "@/services/service";
  ...
  const handleDelete = async (id) => {
    ...
    if (confirmacion) {
      try {
        ...
        const updatedEmpleados = await getAllEmployees();
        if (Array.isArray(updatedEmpleados)) {
          setEmpleados(updatedEmpleados);
        } else {
          console.error("La respuesta de la API no es un array:", updatedEmpleados);
        }
...
<div className="p-6 bg-white shadow-lg rounded-lg">
  <h2 className="text-2xl font-semibold mb-4 text-gray-800">Gestión de Empleados</h2>
  ...
  <table className="table-auto w-full border border-gray-300 rounded-lg">
    <thead className="bg-gray-100">
      <tr>
        ...
        <th className="px-6 py-3 border-b text-left text-sm font-medium text-gray-600">Apellidos</th>
        <th className="px-6 py-3 border-b text-left text-sm font-medium text-gray-600">Correo</th>
        <th className="px-6 py-3 border-b text-left text-sm font-medium text-gray-600">Especialidad</th>
        <th className="px-6 py-3 border-b text-left text-sm font-medium text-gray-600">Sueldo</th>
        <th className="px-6 py-3 border-b text-left text-sm font-medium text-gray-600">Estado</th>
        <th className="px-6 py-3 border-b text-left text-sm font-medium text-gray-600">Fecha de contratación</th>
    ...
    <tbody>
      {Array.isArray(empleados) && empleados.map((usuario) => (
        ...
        >
          <td className="px-6 py-4 border-b text-sm text-gray-700">{usuario.nombre}</td>
          <td className="px-6 py-4 border-b text-sm text-gray-700">{usuario.apellidos}</td>
          <td className="px-6 py-4 border-b text-sm text-gray-700">{usuario.correo}</td>
          <td className="px-6 py-4 border-b text-sm text-gray-700">{usuario.especialidad}</td>
          <td className="px-6 py-4 border-b text-sm text-gray-700">{usuario.sueldo}</td>
          ...
          <td className="px-6 py-4 border-b text-sm text-gray-700">{usuario.fecha_contratacion}</td>
          <td className="px-6 py-4 border-b text-sm text-gray-700">
  ...
  {showModal && (
    <div className="fixed inset-0 bg-gray-600 bg-opacity-50 flex justify-center items-center">
      <div className="bg-white p-8 rounded-lg shadow-xl w-3/4 md:w-1/3">
        ...
        {[
          { label: "Nombre", value: isEditing ? editEmpleado.nombre : newEmpleado.nombre, key: "nombre" },
          { label: "Apellidos", value: isEditing ? editEmpleado.apellidos : newEmpleado.apellidos, key: "apellidos" },
          { label: "Correo", value: isEditing ? editEmpleado.correo : newEmpleado.correo, key: "correo", type: "email" },
          { label: "Especialidad", value: isEditing ? editEmpleado.especialidad : newEmpleado.especialidad, key: "especialidad" },
          { label: "Sueldo", value: isEditing ? editEmpleado.sueldo : newEmpleado.sueldo, key: "sueldo", type: "number" },
          { label: "Fecha de contratación", value: isEditing ? editEmpleado.fecha_contratacion : newEmpleado.fecha_contratacion, key: "fecha_contratacion", type: "date" },
          ...
          <div className="mb-6" key={key}>
            ...
            <input
              ...
              onChange={(e) => {
                ...
                isEditing ? setEditEmpleado(updatedEmpleado) : setNewEmpleado(updatedEmpleado);
              }}
            />
          </div>
        ))}
